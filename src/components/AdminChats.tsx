...
export function AdminChats() {
  ...
  return (
    <div className="grid lg:grid-cols-[360px_1fr] gap-6">
      <div>
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 px-3 py-2 text-[11px] uppercase tracking-[0.2em] border rounded-sm transition ${filter === "all" ? "border-gold bg-gold/10 text-gold" : "border-gold/30 text-muted-foreground hover:border-gold"}`}
          >
            Все ({sessions.length})
          </button>
          <button
            onClick={() => setFilter("tickets")}
            className={`flex-1 px-3 py-2 text-[11px] uppercase tracking-[0.2em] border rounded-sm transition ${filter === "tickets" ? "border-gold bg-gold/10 text-gold" : "border-gold/30 text-muted-foreground hover:border-gold"}`}
          >
            ...
            {visible.map((s) => {
              ...
              return (
                <button
                  key={s.id}
                  onClick={() => setActive(s)}
                  className={`w-full text-left p-3 border rounded-sm transition ${active?.id === s.id ? "border-gold bg-gold/10" : "border-gold/30 hover:border-gold bg-card/40"}`}
                >
      ...
      <div className="border border-gold/30 rounded-sm bg-card/40 min-h-[60vh]">
        {active ? (
    ...
    <div className="flex flex-col h-full max-h-[80vh]">
      <div className="px-5 py-3 border-b border-gold/30 flex items-start justify-between gap-3">
        <div>
          ...
          {ticket && (
            <div className="mt-2 inline-flex items-center gap-2 px-2 py-1 rounded-sm bg-gold/10 border border-gold/40 text-xs text-gold">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-3">
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] px-3 py-2 rounded-sm text-sm whitespace-pre-wrap ${
      ...
      <form onSubmit={(e) => { e.preventDefault(); sendOperator(); }} className="p-3 border-t border-gold/30 flex items-end gap-2">
        ...
        />
        <button type="submit" disabled={!reply.trim() || sending} className="w-10 h-10 inline-flex items-center justify-center rounded-sm bg-gold-gradient text-primary-foreground disabled:opacity-40">
