    ...
    <div className="min-h-[70vh] flex items-center justify-center px-6 py-20">
      <form onSubmit={submit} className="w-full max-w-md p-10 border border-gold/40 rounded-xl bg-card/60 backdrop-blur space-y-5">
        ...
        <label className="block">
          ...
          <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 bg-background/60 border border-gold/30 rounded-xl focus:border-gold focus:outline-none" />
        </label>
        {mode !== "forgot" && (
          <label className="block">
            <span className="block text-xs uppercase tracking-[0.2em] text-muted-foreground mb-2">Пароль</span>
            <input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 bg-background/60 border border-gold/30 rounded-xl focus:border-gold focus:outline-none" />
          </label>
        )}
        <button type="submit" disabled={busy} className="w-full px-6 py-4 bg-gold-gradient text-primary-foreground rounded-xl text-sm uppercase tracking-[0.2em] hover:opacity-95 shadow-gold disabled:opacity-60">
